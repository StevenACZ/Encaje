...
set -euo pipefail
cd -P "$(dirname "$0")/.."
app="${ENCAJE_APP_PATH:-$PWD/build/distribution/Encaje.app}"
output_dir="${ENCAJE_ARTIFACTS_DIR:-$PWD/build/release-artifacts}"
signer="${ENCAJE_SPARKLE_BIN:-$PWD/.build/artifacts/sparkle/Sparkle/bin}/sign_update"
...
repo = 'https://github.com/StevenACZ/Encaje'
rss = ET.Element('rss', version='2.0')
channel = ET.SubElement(rss, 'channel')
ET.SubElement(channel, 'title').text = 'Encaje'
