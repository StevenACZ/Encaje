...
workspace="$(mktemp -d "${TMPDIR:-/tmp}/encaje-verify.XXXXXX")"
mounted=false
cleanup() {
...
PYVERIFY
    [[ "$(/usr/libexec/PlistBuddy -c 'Print :EncajeDevelopmentBuild' "$plist")" == false ]]
    [[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleIdentifier' "$plist")" == com.stevenacz.Encaje ]]
    version="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleShortVersionString' "$plist")"
    build="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$plist")"
    [[ "$version" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ && "$build" =~ ^[1-9][0-9]*$ ]]
    [[ -z "${APP_VERSION:-}" || "$version" == "$APP_VERSION" ]]
    [[ -z "${APP_BUILD:-}" || "$build" == "$APP_BUILD" ]]
    [[ "$(lipo -archs "$app/Contents/MacOS/Encaje")" == arm64 ]]
    otool -l "$app/Contents/MacOS/Encaje" > "$workspace/load-commands"
    ...
    strings "$app/Contents/MacOS/Encaje" > "$workspace/strings"
...
case "$input" in
    ...
    *.dmg)
        ...
        verify_app "$workspace/mount/Encaje.app"
        ;;
    *.zip)
        ...
        verify_app "$workspace/extracted/Encaje.app"
        ;;
